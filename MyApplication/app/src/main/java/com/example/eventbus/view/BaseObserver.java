...
public abstract class BaseObserver<T> extends DisposableObserver<MyResponse> {
...
//        T t = o.getData();
        try {
            ...
            if (myResponse.code ==responseCode.OK) {
                onSuccess(myResponse.data);
            } else {
                if (view != null) {
                    view.onErrorCode(myResponse.error);
                }
                //非  true的所有情况
                onException(PARSE_ERROR, myResponse.data.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
            onError(e.toString());
